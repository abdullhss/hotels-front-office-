    ...
    <>
      <style>{`
        @media print {
          ...
          .reservation-invoice-print-host,
          .reservation-invoice-print-host * { visibility: visible; }
          .reservation-invoice-print-host {
            position: absolute;
            inset: 0;
            z-index: 99999;
          ...
          .reservation-invoice-print-host > button {
            display: none !important;
          }
          .reservation-invoice-no-print { display: none !important; }
          .reservation-invoice-print-host .relative {
            max-height: none !important;
      ...
      <div
        className="reservation-invoice-print-host fixed inset-0 z-50 flex items-center justify-center p-4"
        role="dialog"
        aria-modal="true"
      >
        <button
          type="button"
          className="reservation-invoice-no-print absolute inset-0 bg-black/45"
          onClick={() => onClose?.()}
        ...
        <div className="relative z-10 flex max-h-[min(94vh,900px)] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
          <div className="reservation-invoice-no-print flex shrink-0 items-center justify-between gap-3 border-b border-[#e8ecf2] px-5 py-4">
            <div>
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={handlePrint}
                className="inline-flex items-center gap-2 rounded-xl border border-[#e2e8f0] bg-white px-4 py-2 text-sm font-medium text-[#374151] hover:bg-[#f8fafc]"
              >
                <Printer className="h-4 w-4" />
                {t('roomOperations.invoice.print')}
              </button>
              <button
                type="button"
                onClick={() => onClose?.()}
          ...
          <div ref={printRef} className="min-h-0 flex-1 overflow-y-auto p-5">
            <div className="mb-6 hidden print:block">
              <h1 className="m-0 text-xl font-bold text-[#111827]">
                {t('roomOperations.invoice.title')}
              </h1>
              <p className="m-0 mt-1 text-sm text-[#374151]">
                {t('roomOperations.invoice.assignment', { num: assignmentNum })}
              </p>
            </div>
            ...
            {lines.length === 0 ? (
              ...
              <div className="space-y-6">
                {lines.map((row, index) => {
                  ...
                  return (
                    <article
                      ...
                      className="rounded-2xl border border-[#e2e8f0] bg-[#fafbfd] p-4 print:break-inside-avoid"
                    >
                ...
                <div className="flex items-center justify-between rounded-2xl border-2 border-brand-primary/20 bg-[#eef2ff] px-5 py-4 print:break-inside-avoid">
          ...
          <div className="reservation-invoice-no-print flex shrink-0 justify-end border-t border-[#e8ecf2] px-5 py-4">
            <button
              type="button"
              onClick={() => onClose?.()}
              className="rounded-xl bg-brand-primary px-6 py-2.5 text-sm font-medium text-white hover:bg-brand-primary-hover"
            >
              {t('roomOperations.invoice.closeAndRefresh')}
            </button>
          </div>
        </div>
      </div>
